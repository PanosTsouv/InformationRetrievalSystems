...
/**
 * A {@link Similarity} based on document embeddings generated by averaging
 * {@link Word2Vec} word vectors. Averaging can be done by simple "mean", but
 * also using index stats like tf, idf and tf-idf.
 */
public class WordEmbeddingsSimilarity extends Similarity {
  ...
  private class EmbeddingsSimScorer extends SimScorer {
    ...
    public float score(int doc, float freq) throws IOException {
      ...
      INDArray denseDocumentVector = VectorizeUtils.toDenseAverageVector(reader.getTermVector(doc, fieldName),
          reader.numDocs(), word2Vec, smoothing);
      Float value = (float) Transforms.cosineSim(denseQueryVector, denseDocumentVector);
      //System.out.println("Doc with id " + doc +  " has score " + value );
      if(value.isNaN())
      {
        value = 0.0f;
      }
    ...
    public INDArray getQueryVector() throws IOException {
