...
public class SearchIndex {
    public SearchIndex(String indexPath, String field, String similarity, float lamda) throws IOException, ParseException
    {
        int[] hits = {20,30,50};
        IndexReader indexReader = DirectoryReader.open(FSDirectory.open(Paths.get(indexPath)));
        IndexSearcher indexSearcher = new IndexSearcher(indexReader);
        if(similarity.equals("BM25Similarity"))
        {
            indexSearcher.setSimilarity(new BM25Similarity());
        }
        else if(similarity.equals("LMJelinekMercerSimilarity"))
        {
            indexSearcher.setSimilarity(new LMJelinekMercerSimilarity(lamda));
        }
        else
        {
